...
static void test_sentence_count(void) {
    ...
    // Argument validation
    ATT_ASSERT_STATUS(mjb_sentence_count("A", 1, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, NULL, NULL),
        MJB_STATUS_INVALID_ARGUMENT, "Count rejects NULL count");
    ATT_ASSERT_STATUS(mjb_sentence_count(NULL, 1, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
        MJB_STATUS_INVALID_ARGUMENT, "Count rejects NULL buffer");
    ATT_ASSERT(count, (size_t)0, "Count is zero after NULL buffer");

    count = 6251;
    ATT_ASSERT_STATUS(mjb_sentence_count("A", 1, MJB_ENC_UNKNOWN, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_sentence_count("", 0, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
        "Count: empty string status");
    ATT_ASSERT(count, (size_t)0, "Count: empty string");
    ATT_ASSERT_STATUS(mjb_sentence_count(NULL, 0, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
        "Count: NULL buffer with zero length status");

    mjb_diagnostic diagnostic;
    ATT_ASSERT_STATUS(mjb_sentence_count("A\x80", 2, MJB_ENC_UTF_8, MJB_MALFORMED_STOP,
                          &count, &diagnostic),
        MJB_STATUS_MALFORMED_INPUT, "Count rejects malformed input");
    ATT_ASSERT(count, (size_t)0, "Count is zero after malformed input");
    ATT_ASSERT(diagnostic.byte_offset, (size_t)1, "Sentence count malformed offset");

    ATT_ASSERT_STATUS(mjb_sentence_count("A\x80", 2, MJB_ENC_UTF_8,
                          MJB_MALFORMED_REPLACE, &count, &diagnostic),
        MJB_STATUS_OK, "Sentence count replaces malformed input");
    ATT_ASSERT(count, (size_t)1, "Replacement remains in one sentence");
    ATT_ASSERT(diagnostic.byte_offset, (size_t)1,
        "Sentence replacement retains malformed offset");

    ATT_ASSERT_STATUS(mjb_sentence_count("A\x80", 2, MJB_ENC_UTF_8, MJB_MALFORMED_SKIP,
                          &count, &diagnostic),
        MJB_STATUS_OK, "Sentence count skips malformed input");
    ATT_ASSERT(count, (size_t)1, "Skipping malformed input remains one sentence");

    // Text without a terminator is a single segment
    ATT_ASSERT_STATUS(mjb_sentence_count("One sentence", 12, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
        MJB_STATUS_OK, "Count: one sentence status");
    ATT_ASSERT(count, (size_t)1, "Count: one sentence");

    // Terminators split segments (SB4, SB11)
    ATT_ASSERT_STATUS(mjb_sentence_count("Hello. How are you? Fine!", 25, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
        MJB_STATUS_OK, "Count: three sentences status");
    ATT_ASSERT(count, (size_t)3, "Count: three sentences");
    ATT_ASSERT_STATUS(mjb_sentence_count("One.\nTwo.\n", 10, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_sentence_count("Hi.   ", 6, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
    ...
    ATT_ASSERT_STATUS(mjb_sentence_count("Wait... then more.", 18, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_sentence_count("Dr. Smith went.", 15, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_sentence_count("Hi. Bye.", MJB_NUL_TERMINATED, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_sentence_count(utf16le_two, 10, MJB_ENC_UTF_16LE, MJB_MALFORMED_STOP, &count, NULL),
        MJB_STATUS_OK, "Count: UTF-16LE status");
    ATT_ASSERT(count, (size_t)2, "Count: UTF-16LE two sentences");
}

ATT_TEST(break_sentence) {
    mjb_next_sentence_state state;
    state.index = 0;
