...
void break_sentence_callback(const char *buffer, size_t size, unsigned int current_line, mjb_break_type *expected_types) {
    ...
    while((bt = mjb_break_sentence(buffer, size, MJB_ENC_UTF_8, &state)) != MJB_BT_NOT_SET) {
...
void *test_break_sentence(void *arg) {
    ...
    ATT_ASSERT((uint8_t)mjb_break_sentence(NULL, 1, MJB_ENC_UTF_8, &state),
        (uint8_t)MJB_BT_NOT_SET, "Sentence break rejects NULL buffer")
    ATT_ASSERT((uint8_t)mjb_break_sentence("A", 1, MJB_ENC_UTF_8, NULL),
