...
int test_next(void *arg) {
    ATT_ASSERT_STATUS(mjb_for_each_codepoint(NULL, 1, MJB_ENC_UTF_8, next_character),
        MJB_STATUS_INVALID_ARGUMENT, "NULL next buffer")
    ATT_ASSERT_STATUS(mjb_for_each_codepoint("A", 1, MJB_ENC_UTF_8, NULL),
        MJB_STATUS_INVALID_ARGUMENT, "NULL next callback")
    ATT_ASSERT_STATUS(mjb_for_each_codepoint("Hèllò", 7, MJB_ENC_UTF_8, next_character),
        MJB_STATUS_OK, "Next character")
    ATT_ASSERT(mjb_test_count, 5, "mjb_for_each_codepoint")

    mjb_test_count = 0;
    ATT_ASSERT_STATUS(mjb_for_each_codepoint("A", 1, MJB_ENC_UTF_8, stop_next_character),
