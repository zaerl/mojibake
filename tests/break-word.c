...
void break_word_callback(const char *buffer, size_t byte_length, unsigned int current_line, mjb_break_type *expected_types) {
    char test_name[256];
 ...
 static void test_truncate_word(void) {
    ...
    // Edge cases
    ATT_ASSERT(mjb_truncate_word("", 0, MJB_ENC_UTF_8, 3), (size_t)0,
        "Truncate word: empty string")
    ...
    ATT_ASSERT(mjb_truncate_word_width("Hello World", 11, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_WESTERN,
        0), (size_t)0, "Truncate word width: 0 columns")

    // "Hello"=5 cols, " "=1 col would exceed 5
    ATT_ASSERT(mjb_truncate_word_width("Hello World", 11, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_WESTERN,
        5), (size_t)5, "Truncate word width: 5 columns")

    // "Hello " = 6 cols, "World" would exceed 6
    ATT_ASSERT(mjb_truncate_word_width("Hello World", 11, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_WESTERN,
        6), (size_t)6, "Truncate word width: 6 columns")
    ATT_ASSERT(mjb_truncate_word_width("Hello World", 11, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_WESTERN,
        11), (size_t)11, "Truncate word width: 11 columns (no-op)")
