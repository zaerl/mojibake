...
static void test_word_count(void) {
    ...
    // Argument validation
    ATT_ASSERT_STATUS(mjb_word_count("A", 1, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, NULL, NULL),
        MJB_STATUS_INVALID_ARGUMENT, "Count rejects NULL count")
    ATT_ASSERT_STATUS(mjb_word_count(NULL, 1, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
        MJB_STATUS_INVALID_ARGUMENT, "Count rejects NULL buffer")
    ATT_ASSERT(count, (size_t)0, "Count is zero after NULL buffer")

    count = 6251;
    ATT_ASSERT_STATUS(mjb_word_count("A", 1, MJB_ENC_UNKNOWN, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_word_count("", 0, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
        "Count: empty string status")
    ATT_ASSERT(count, (size_t)0, "Count: empty string")
    ATT_ASSERT_STATUS(mjb_word_count(NULL, 0, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
        "Count: NULL buffer with zero length status")

    mjb_diagnostic diagnostic;
    ATT_ASSERT_STATUS(mjb_word_count("A\x80", 2, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count,
                          &diagnostic),
        MJB_STATUS_MALFORMED_INPUT, "Count rejects malformed input")
    ATT_ASSERT(count, (size_t)0, "Count is zero after malformed input")
    ATT_ASSERT(diagnostic.byte_offset, (size_t)1, "Word count malformed offset")

    const char malformed_word[] = "A\x80"
                                  "B";
    ATT_ASSERT_STATUS(mjb_word_count(malformed_word, sizeof(malformed_word) - 1, MJB_ENC_UTF_8,
                          MJB_MALFORMED_REPLACE, &count, &diagnostic),
        MJB_STATUS_OK, "Word count replaces malformed input")
    ATT_ASSERT(count, (size_t)2, "Replacement separates word segments")
    ATT_ASSERT(diagnostic.byte_offset, (size_t)1, "Word replacement retains malformed offset")

    ATT_ASSERT_STATUS(mjb_word_count(malformed_word, sizeof(malformed_word) - 1, MJB_ENC_UTF_8,
                          MJB_MALFORMED_SKIP, &count, &diagnostic),
        MJB_STATUS_OK, "Word count skips malformed input")
    ATT_ASSERT(count, (size_t)1, "Skipping malformed input joins adjacent letters")

    // Punctuation, whitespace, and symbol segments are not counted
    ATT_ASSERT_STATUS(mjb_word_count("Hello, world! It works.", 23, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
        MJB_STATUS_OK, "Count: sentence status")
    ATT_ASSERT(count, (size_t)4, "Count: sentence has four words")
    ATT_ASSERT_STATUS(mjb_word_count("...", 3, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
    ...
    ATT_ASSERT_STATUS(mjb_word_count("don't stop", 10, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
        "Count: apostrophe status")
    ATT_ASSERT(count, (size_t)2, "Count: apostrophe stays inside the word")
    ATT_ASSERT_STATUS(mjb_word_count("e.g. example.com", 16, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_word_count("state-of-the-art", 16, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_word_count("3.14 is pi", 10, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
        "Count: number status")
    ...
    ATT_ASSERT_STATUS(mjb_word_count("foo_bar baz", 11, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
        "Count: connector status")
    ATT_ASSERT(count, (size_t)2, "Count: connector joins one word")
    ATT_ASSERT_STATUS(mjb_word_count("_ _", 3, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
        "Count: connector-only status")
    ATT_ASSERT(count, (size_t)0, "Count: connector-only segments are not words")

    // Emoji are not word-like
    ATT_ASSERT_STATUS(mjb_word_count("a \xF0\x9F\x91\x8D b", 8, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_word_count("\xE6\x9D\xB1\xE4\xBA\xAC", 6, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_word_count("\xE3\x82\xAB\xE3\x82\xBF\xE3\x82\xAB\xE3\x83\x8A", 12,
        MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK, "Count: katakana status")
    ATT_ASSERT(count, (size_t)1, "Count: katakana run is one word")

    // MJB_NUL_TERMINATED requests a terminator scan
    ATT_ASSERT_STATUS(mjb_word_count("Hi there", MJB_NUL_TERMINATED, MJB_ENC_UTF_8, MJB_MALFORMED_STOP, &count, NULL),
    ...
    ATT_ASSERT_STATUS(mjb_word_count(utf16le_two, 12, MJB_ENC_UTF_16LE, MJB_MALFORMED_STOP, &count, NULL), MJB_STATUS_OK,
