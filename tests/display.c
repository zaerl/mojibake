...
int test_display(void *arg) {
    ...
    ATT_ASSERT_STATUS(mjb_display_width("e\xCC\x81", 3, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_AUTO, &sw),
        MJB_STATUS_OK, "e + combining acute")
    ATT_ASSERT(sw, 1, "e + combining acute (é)")

    ATT_ASSERT_STATUS(mjb_display_width("\xCC\x81", 2, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_AUTO, &sw),
        MJB_STATUS_OK, "Combining acute alone")
    ATT_ASSERT(sw, 0, "Combining acute alone")

    // Test zero-width format characters
    ATT_ASSERT_STATUS(mjb_display_width("\xE2\x80\x8B", 3, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_AUTO,
                          &sw),
        MJB_STATUS_OK, "Zero-width space")
    ATT_ASSERT(sw, 0, "Zero-width space (U+200B)")

    ATT_ASSERT_STATUS(mjb_display_width("\xE2\x80\x8C", 3, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_AUTO,
                          &sw),
        MJB_STATUS_OK, "Zero-width non-joiner")
    ATT_ASSERT(sw, 0, "Zero-width non-joiner (U+200C)")

    ATT_ASSERT_STATUS(mjb_display_width("\xE2\x80\x8D", 3, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_AUTO,
                          &sw),
        MJB_STATUS_OK, "Zero-width joiner")
    ...
    ATT_ASSERT_STATUS(mjb_display_width("\xe4\xb8\xad", 3, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_AUTO,
                          &sw),
        MJB_STATUS_OK, "CJK ideograph")
    ATT_ASSERT(sw, 2, "CJK ideograph (中)")

    ATT_ASSERT_STATUS(mjb_display_width("\xe4\xb8\xad\xe6\x96\x87", 6, MJB_ENC_UTF_8,
                          MJB_WIDTH_CONTEXT_AUTO, &sw),
        MJB_STATUS_OK, "Two CJK ideographs")
    ATT_ASSERT(sw, 4, "Two CJK ideographs (中文)")

    // Test full-width characters
    ATT_ASSERT_STATUS(mjb_display_width("\xef\xbc\xa1", 3, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_AUTO,
                          &sw),
        MJB_STATUS_OK, "Full-width A")
    ...
    ATT_ASSERT_STATUS(mjb_display_width("\xe2\x9d\xa4\xef\xb8\x8f", 6, MJB_ENC_UTF_8,
                          MJB_WIDTH_CONTEXT_AUTO, &sw),
        MJB_STATUS_OK, "Emoji with variation selector")
    ...
    // TYPE-4)
    ATT_ASSERT_STATUS(mjb_display_width("\xf0\x9f\x91\x8b\xf0\x9f\x8f\xbd", 8, MJB_ENC_UTF_8,
                          MJB_WIDTH_CONTEXT_AUTO, &sw),
        MJB_STATUS_OK, "Emoji with skin tone")
    ATT_ASSERT(sw, 4, "Emoji with skin tone (👋🏽) - counts base + modifier separately")

    // ZWJ sequence: 👨‍👩‍👧 (U+1F468 MAN + U+200D ZWJ + U+1F469 WOMAN + U+200D ZWJ + U+1F467
    // GIRL)
    ATT_ASSERT_STATUS(mjb_display_width("\xf0\x9f\x91\xa8\xe2\x80\x8d\xf0\x9f\x91\xa9\xe2\x80\x8d"
                                        "\xf0\x9f\x91\xa7",
                          18, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_AUTO, &sw),
        MJB_STATUS_OK, "Family emoji ZWJ sequence")
    ATT_ASSERT(sw, 6,
        "Family emoji ZWJ sequence (👨‍👩‍👧) - counts each emoji separately")

    // Auto context with Western text → narrow (1)
    ATT_ASSERT_STATUS(mjb_display_width("Hello \xc2\xa1Hola!", 13, MJB_ENC_UTF_8,
                          MJB_WIDTH_CONTEXT_AUTO, &sw),
        MJB_STATUS_OK, "Ambiguous in Western text (AUTO)")
    ATT_ASSERT(sw, 12, "Ambiguous in Western text (AUTO) - treats as narrow")

    // AUTO context with CJK text → wide (2)
    ATT_ASSERT_STATUS(mjb_display_width("\xe4\xb8\xad\xe6\x96\x87\xc2\xa1\xe4\xb8\xad", 11,
                          MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_AUTO, &sw),
        MJB_STATUS_OK, "ambiguous in CJK text (AUTO)")
    ATT_ASSERT(sw, 8, "Ambiguous in CJK text (AUTO) - treats as wide: 中(2)+文(2)+¡(2)+中(2)")

    // Explicit WESTERN context → narrow (1)
    ATT_ASSERT_STATUS(mjb_display_width("\xc2\xa1", 2, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_WESTERN,
                          &sw),
        MJB_STATUS_OK, "Ambiguous (WESTERN)")
    ATT_ASSERT(sw, 1, "Ambiguous (WESTERN) - narrow")

    // Explicit EAST_ASIAN context → wide (2)
    ATT_ASSERT_STATUS(mjb_display_width("\xc2\xa1", 2, MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_EAST_ASIAN,
                          &sw),
        MJB_STATUS_OK, "Ambiguous (EAST_ASIAN)")
