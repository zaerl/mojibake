...
int test_example(void *arg) {
    ...
    // This function is automatically generated. Do not edit.
    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+%04X lowercase: U+%04X", character.codepoint,
            character.lowercase); // Added by the script
        ATT_ASSERT(test_buffer, "U+022A lowercase: U+022B",
            "mjb_codepoint_character test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "NFC: %.*s", (int)result.output_size,
            result.output);                                               // Added by the script
...

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Filtered: %.*s", (int)result.output_size,
            result.output); // Added by the script
        ATT_ASSERT(test_buffer, "Filtered: Hello world",
            "mjb_string_filter test failed") // Added by the script
...

    {
        ...
        const char *input = "Stra\xC3\x9F"
                            "e\xC2\xAD";
        mjb_result result;

        if(mjb_nfkc_casefold(input, strlen(input), MJB_ENC_UTF_8, MJB_ENC_UTF_8, &result) !=
            MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "%.*s", (int)result.output_size,
            result.output);                                                 // Added by the script
...

    {
        ...
        mjb_quick_check_result check = mjb_string_is_normalized(input, strlen(input), MJB_ENC_UTF_8,
            MJB_NORMALIZATION_NFC);

        // NFC normalized: yes
        // printf("NFC normalized: %s", check == MJB_QC_YES ? "yes" : "no");
        snprintf(test_buffer, sizeof(test_buffer), "NFC normalized: %s",
            check == MJB_QC_YES ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "NFC normalized: yes",
            "mjb_string_is_normalized test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "UTF-16LE detected: %s",
            is_utf16le ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "UTF-16LE detected: yes",
            "mjb_string_encoding test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "ASCII: %s",
            mjb_string_is_ascii(input, strlen(input)) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "ASCII: yes",
            "mjb_string_is_ascii test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Valid UTF-8: %s",
            mjb_string_is_utf8(input, strlen(input)) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Valid UTF-8: yes",
            "mjb_string_is_utf8 test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "UTF-16: %s",
            mjb_string_is_utf16(utf16be, sizeof(utf16be) - 1) ? "yes" :
                                                                "no"); // Added by the script
        ATT_ASSERT(test_buffer, "UTF-16: yes",
            "mjb_string_is_utf16 test failed") // Added by the script
    }

    {
        // Example for mjb_string_length
        MJB_TEST_COVERAGE(mjb_string_length); // Added by the script
        // The "Héllö" string is five Unicode characters, but has different byte lengths in
        // different encodings.
        ...
        snprintf(test_buffer, sizeof(test_buffer), "%zu UTF-8 characters",
            mjb_string_length(utf8, 7, MJB_ENC_UTF_8)); // Added by the script
        ATT_ASSERT(test_buffer, "5 UTF-8 characters",
            "mjb_string_length test failed") // Added by the script
        // 5 UTF-16LE characters
        // printf("%zu UTF-16LE characters", mjb_string_length(utf16le, 10, MJB_ENC_UTF_16LE));
        snprintf(test_buffer, sizeof(test_buffer), "%zu UTF-16LE characters",
            mjb_string_length(utf16le, 10, MJB_ENC_UTF_16LE)); // Added by the script
        ATT_ASSERT(test_buffer, "5 UTF-16LE characters",
            "mjb_string_length test failed") // Added by the script
        // 5 UTF-16BE characters
        // printf("%zu UTF-16BE characters", mjb_string_length(utf16be, 10, MJB_ENC_UTF_16BE));
        snprintf(test_buffer, sizeof(test_buffer), "%zu UTF-16BE characters",
            mjb_string_length(utf16be, 10, MJB_ENC_UTF_16BE)); // Added by the script
        ATT_ASSERT(test_buffer, "5 UTF-16BE characters",
            "mjb_string_length test failed") // Added by the script
        // 5 UTF-32LE characters
        // printf("%zu UTF-32LE characters", mjb_string_length(utf32le, 20, MJB_ENC_UTF_32LE));
        snprintf(test_buffer, sizeof(test_buffer), "%zu UTF-32LE characters",
            mjb_string_length(utf32le, 20, MJB_ENC_UTF_32LE)); // Added by the script
        ATT_ASSERT(test_buffer, "5 UTF-32LE characters",
            "mjb_string_length test failed") // Added by the script
        // 5 UTF-32BE characters
        // printf("%zu UTF-32BE characters", mjb_string_length(utf32be, 20, MJB_ENC_UTF_32BE));
        snprintf(test_buffer, sizeof(test_buffer), "%zu UTF-32BE characters",
            mjb_string_length(utf32be, 20, MJB_ENC_UTF_32BE)); // Added by the script
        ATT_ASSERT(test_buffer, "5 UTF-32BE characters",
            "mjb_string_length test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "A callback is required: %s",
            callback_required ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "A callback is required: yes",
            "mjb_string_each_character test failed") // Added by the script
    }

    {
        ...
        if(mjb_codepoint_property_binary('A', MJB_PR_ALPHABETIC, &is_alphabetic) != MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+0041 is alphabetic: %s",
            is_alphabetic ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+0041 is alphabetic: yes",
            "mjb_codepoint_property_binary test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+0041 uses the Latin script: %s",
            script == MJB_SC_LATN ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+0041 uses the Latin script: yes",
            "mjb_codepoint_property_int test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "decimal=%d, digit=%d, numeric=%s", num.decimal,
            num.digit, num.numeric); // Added by the script
        ATT_ASSERT(test_buffer, "decimal=1, digit=1, numeric=1",
            "mjb_codepoint_numeric_value test failed") // Added by the script
        ...
        snprintf(test_buffer, sizeof(test_buffer), "decimal=%d, digit=%d, numeric=%s", num.decimal,
            num.digit, num.numeric); // Added by the script
        ATT_ASSERT(test_buffer, "decimal=-1, digit=-1, numeric=1/2",
            "mjb_codepoint_numeric_value test failed") // Added by the script
    }

    {
        ...
        ATT_ASSERT(test_buffer, "Block: Basic Latin",
            "mjb_codepoint_block test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Greek script: %s",
            script == MJB_SC_GREK ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Greek script: yes",
            "mjb_codepoint_script test failed") // Added by the script
    }

    {
        ...
        if(count > 3 || mjb_codepoint_script_extensions(0x30FC, scripts, &count) != MJB_STATUS_OK) {
            ATT_ASSERT(0, 1, "mjb_codepoint_script_extensions test failed") // Added by the script
            return 1;
        }
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+30FC has %zu Script_Extensions",
            count); // Added by the script
        ATT_ASSERT(test_buffer, "U+30FC has 2 Script_Extensions",
            "mjb_codepoint_script_extensions test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "%.*s sign uses %u UTF-8 bytes", (int)size,
            encoded, size); // Added by the script
        ATT_ASSERT(test_buffer, "€ sign uses 3 UTF-8 bytes",
            "mjb_codepoint_encode test failed") // Added by the script
    }

    {
        ...
        if(mjb_string_convert_encoding(input, strlen(input), MJB_ENC_UTF_8, MJB_ENC_UTF_16LE,
               &result) != MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "UTF-16LE bytes: %zu",
            result.output_size); // Added by the script
        ATT_ASSERT(test_buffer, "UTF-16LE bytes: 8",
            "mjb_string_convert_encoding test failed") // Added by the script
        mjb_result_free(&result);
    }

    {
        ...
        int order = mjb_string_compare("apple", 5, MJB_ENC_UTF_8, "banana", 6, MJB_ENC_UTF_8,
            MJB_COLLATION_NON_IGNORABLE);

        // apple sorts before banana: yes
        // printf("apple sorts before banana: %s", order < 0 ? "yes" : "no");
        snprintf(test_buffer, sizeof(test_buffer), "apple sorts before banana: %s",
            order < 0 ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "apple sorts before banana: yes",
            "mjb_string_compare test failed") // Added by the script
    }

    {
        ...
        if(mjb_collation_key("r\xC3\xA9sum\xC3\xA9", 8, MJB_ENC_UTF_8, MJB_COLLATION_NON_IGNORABLE,
               &key) != MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Sort key is non-empty: %s",
            key.output_size > 0 ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Sort key is non-empty: yes",
            "mjb_collation_key test failed") // Added by the script
        mjb_result_free(&key);
    }

    {
        ...
        if(mjb_case(input, strlen(input), MJB_ENC_UTF_8, MJB_CASE_UPPER, MJB_ENC_UTF_8, &result) !=
            MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Upper: %.*s", (int)result.output_size,
            result.output);                                               // Added by the script
...

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+10FFFD valid: %s",
            mjb_codepoint_is_valid(0x10FFFD) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+10FFFD valid: yes",
            "mjb_codepoint_is_valid test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Letter A is graphic: %s",
            mjb_codepoint_is_graphic('A') ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Letter A is graphic: yes",
            "mjb_codepoint_is_graphic test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+0301 is combining: %s",
            mjb_codepoint_is_combining(0x0301) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+0301 is combining: yes",
            "mjb_codepoint_is_combining test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+1100 is a leading Jamo: %s",
            mjb_codepoint_is_hangul_l(0x1100) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+1100 is a leading Jamo: yes",
            "mjb_codepoint_is_hangul_l test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+1161 is a vowel Jamo: %s",
            mjb_codepoint_is_hangul_v(0x1161) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+1161 is a vowel Jamo: yes",
            "mjb_codepoint_is_hangul_v test failed") // Added by the script
    }

    {
        ...
        // printf("U+11A8 is a trailing Jamo: %s", mjb_codepoint_is_hangul_t(0x11A8) ? "yes" :
        // "no");
        snprintf(test_buffer, sizeof(test_buffer), "U+11A8 is a trailing Jamo: %s",
            mjb_codepoint_is_hangul_t(0x11A8) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+11A8 is a trailing Jamo: yes",
            "mjb_codepoint_is_hangul_t test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+1100 is Hangul Jamo: %s",
            mjb_codepoint_is_hangul_jamo(0x1100) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+1100 is Hangul Jamo: yes",
            "mjb_codepoint_is_hangul_jamo test failed") // Added by the script
    }

    {
        ...
        // printf("U+AC00 is a Hangul syllable: %s", mjb_codepoint_is_hangul_syllable(0xAC00) ?
        // "yes" : "no");
        snprintf(test_buffer, sizeof(test_buffer), "U+AC00 is a Hangul syllable: %s",
            mjb_codepoint_is_hangul_syllable(0xAC00) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+AC00 is a Hangul syllable: yes",
            "mjb_codepoint_is_hangul_syllable test failed") // Added by the script
    }

    {
        ...
        // printf("U+4E00 is a CJK ideograph: %s", mjb_codepoint_is_cjk_ideograph(0x4E00) ? "yes" :
        // "no");
        snprintf(test_buffer, sizeof(test_buffer), "U+4E00 is a CJK ideograph: %s",
            mjb_codepoint_is_cjk_ideograph(0x4E00) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+4E00 is a CJK ideograph: yes",
            "mjb_codepoint_is_cjk_ideograph test failed") // Added by the script
    }

    {
        ...
        // printf("U+20000 is a CJK extension ideograph: %s", mjb_codepoint_is_cjk_ext(0x20000) ?
        // "yes" : "no");
        snprintf(test_buffer, sizeof(test_buffer), "U+20000 is a CJK extension ideograph: %s",
            mjb_codepoint_is_cjk_ext(0x20000) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+20000 is a CJK extension ideograph: yes",
            "mjb_codepoint_is_cjk_ext test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Uppercase letters are graphic: %s",
            graphic ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Uppercase letters are graphic: yes",
            "mjb_category_is_graphic test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Nonspacing marks are combining: %s",
            combining ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Nonspacing marks are combining: yes",
            "mjb_category_is_combining test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "%c > %c", 'A',
            codepoint); // Added by the script
        ATT_ASSERT(test_buffer, "A > a",
            "mjb_codepoint_to_lowercase test failed") // Added by the script

        codepoint = mjb_codepoint_to_lowercase(0x03A3); // U+03A3 = 'Σ'

        // U+03A3 > U+03C3, Σ > σ
        // printf("U+%04X > U+%04X, %s > %s",  0x03A3, codepoint, "Σ", "σ");
        snprintf(test_buffer, sizeof(test_buffer), "U+%04X > U+%04X, %s > %s", 0x03A3, codepoint,
            "Σ", "σ"); // Added by the script
        ATT_ASSERT(test_buffer, "U+03A3 > U+03C3, Σ > σ",
            "mjb_codepoint_to_lowercase test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Uppercase: U+%04X",
            uppercase); // Added by the script
        ATT_ASSERT(test_buffer, "Uppercase: U+00C9",
            "mjb_codepoint_to_uppercase test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Titlecase: U+%04X",
            titlecase); // Added by the script
        ATT_ASSERT(test_buffer, "Titlecase: U+01F2",
            "mjb_codepoint_to_titlecase test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "First line-break result is set: %s",
            type != MJB_BT_NOT_SET ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "First line-break result is set: yes",
            "mjb_break_line test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Word-break positions: %zu",
            boundaries); // Added by the script
        ATT_ASSERT(test_buffer, "Word-break positions: 11",
            "mjb_break_word test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Sentence-break positions: %zu",
            boundaries); // Added by the script
        ATT_ASSERT(test_buffer, "Sentence-break positions: 15",
            "mjb_break_sentence test failed") // Added by the script
    }

    {
        ...
        while(mjb_break_grapheme_cluster(input, strlen(input), MJB_ENC_UTF_8, &state) !=
            MJB_BT_NOT_SET) {
            ++codepoints;
        }
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Codepoints examined: %zu",
            codepoints); // Added by the script
        ATT_ASSERT(test_buffer, "Codepoints examined: 2",
            "mjb_break_grapheme_cluster test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "First two graphemes use %zu bytes",
            bytes); // Added by the script
        ATT_ASSERT(test_buffer, "First two graphemes use 9 bytes",
            "mjb_truncate test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Two columns include %zu byte",
            bytes); // Added by the script
        ATT_ASSERT(test_buffer, "Two columns include 1 byte",
            "mjb_truncate_width test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "First word segment uses %zu bytes",
            bytes); // Added by the script
        ATT_ASSERT(test_buffer, "First word segment uses 5 bytes",
            "mjb_truncate_word test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Six columns include %zu bytes",
            bytes); // Added by the script
        ATT_ASSERT(test_buffer, "Six columns include 6 bytes",
            "mjb_truncate_word_width test failed") // Added by the script
    }

    {
        ...
        if(mjb_bidi_resolve(input, strlen(input), MJB_ENC_UTF_8, MJB_DIRECTION_AUTO, &paragraph) !=
            MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Paragraph codepoints: %zu",
            paragraph.count); // Added by the script
        ATT_ASSERT(test_buffer, "Paragraph codepoints: 7",
            "mjb_bidi_resolve test failed") // Added by the script
...

    {
        ...
        if(mjb_bidi_resolve(input, strlen(input), MJB_ENC_UTF_8, MJB_DIRECTION_AUTO, &paragraph) !=
                MJB_STATUS_OK ||
            mjb_bidi_reorder_line(&paragraph, 0, paragraph.count, visual_order) != MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "First visual index: %zu",
            visual_order[0]); // Added by the script
        ATT_ASSERT(test_buffer, "First visual index: 2",
            "mjb_bidi_reorder_line test failed") // Added by the script
        mjb_bidi_free(&paragraph);
    }

    {
        ...
        if(mjb_bidi_resolve("abc", 3, MJB_ENC_UTF_8, MJB_DIRECTION_LTR, &paragraph) !=
                MJB_STATUS_OK ||
            mjb_bidi_reorder_line(&paragraph, 0, 3, visual_order) != MJB_STATUS_OK ||
            mjb_bidi_line_runs(&paragraph, visual_order, 3, NULL, &run_count) != MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Visual runs: %zu",
            run_count); // Added by the script
        ATT_ASSERT(test_buffer, "Visual runs: 1",
            "mjb_bidi_line_runs test failed") // Added by the script
        mjb_bidi_free(&paragraph);
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Paragraph released: %s",
            paragraph.chars == NULL ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Paragraph released: yes",
            "mjb_bidi_free test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Greek alpha starts an identifier: %s",
            starts ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Greek alpha starts an identifier: yes",
            "mjb_codepoint_is_id_start test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Digit 7 continues an identifier: %s",
            continues ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Digit 7 continues an identifier: yes",
            "mjb_codepoint_is_id_continue test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Letter A is XID_Start: %s",
            mjb_codepoint_is_xid_start('A') ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Letter A is XID_Start: yes",
            "mjb_codepoint_is_xid_start test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Underscore is XID_Continue: %s",
            continues ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Underscore is XID_Continue: yes",
            "mjb_codepoint_is_xid_continue test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Plus sign is Pattern_Syntax: %s",
            syntax ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Plus sign is Pattern_Syntax: yes",
            "mjb_codepoint_is_pattern_syntax test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Space is Pattern_White_Space: %s",
            whitespace ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Space is Pattern_White_Space: yes",
            "mjb_codepoint_is_pattern_white_space test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Valid identifier: %s",
            valid ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Valid identifier: yes",
            "mjb_string_is_identifier test failed") // Added by the script
    }

    {
        ...
        ATT_ASSERT(test_buffer, "Property: Alphabetic",
            "mjb_property_name test failed") // Added by the script
    }

    {
        ...
        if(mjb_confusable_skeleton(input, strlen(input), MJB_ENC_UTF_8, MJB_ENC_UTF_8, &result) !=
            MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "%.*s", (int)result.output_size,
            result.output); // Added by the script
        ATT_ASSERT(test_buffer, "hello",
            "mjb_confusable_skeleton test failed") // Added by the script
        mjb_result_free(&result);
    }

    {
        ...
        bool confusable = mjb_string_is_confusable(latin, strlen(latin), MJB_ENC_UTF_8, mixed,
            strlen(mixed), MJB_ENC_UTF_8);

        // Visually confusable: yes
        // printf("Visually confusable: %s", confusable ? "yes" : "no");
        snprintf(test_buffer, sizeof(test_buffer), "Visually confusable: %s",
            confusable ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Visually confusable: yes",
            "mjb_string_is_confusable test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+1F600 has Emoji_Presentation: %s",
            emoji.presentation ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+1F600 has Emoji_Presentation: yes",
            "mjb_codepoint_emoji test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Number sign has the Emoji property: %s",
            emoji ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Number sign has the Emoji property: yes",
            "mjb_codepoint_is_emoji test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer),
            "Grinning face defaults to emoji presentation: %s",
            presentation ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Grinning face defaults to emoji presentation: yes",
            "mjb_codepoint_is_emoji_presentation test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Medium skin tone is an emoji modifier: %s",
            modifier ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Medium skin tone is an emoji modifier: yes",
            "mjb_codepoint_is_emoji_modifier test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Waving hand accepts an emoji modifier: %s",
            modifier_base ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Waving hand accepts an emoji modifier: yes",
            "mjb_codepoint_is_emoji_modifier_base test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Zero-width joiner is an emoji component: %s",
            component ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Zero-width joiner is an emoji component: yes",
            "mjb_codepoint_is_emoji_component test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Red heart is Extended_Pictographic: %s",
            pictographic ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Red heart is Extended_Pictographic: yes",
            "mjb_codepoint_is_extended_pictographic test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+1F600 is in the SMP: %s",
            plane == MJB_PLANE_SMP ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+1F600 is in the SMP: yes",
            "mjb_codepoint_plane test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Plane 16 is valid: %s",
            mjb_plane_is_valid(MJB_PLANE_PUA_B) ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Plane 16 is valid: yes",
            "mjb_plane_is_valid test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Plane: %s",
            mjb_plane_name(MJB_PLANE_BMP, false)); // Added by the script
        ATT_ASSERT(test_buffer, "Plane: Basic Multilingual Plane",
            "mjb_plane_name test failed") // Added by the script
    }

    {
        ...
        if(mjb_string_emoji_sequence(flag, strlen(flag), MJB_ENC_UTF_8, &emoji) != MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Sequence codepoints: %zu",
            emoji.codepoint_count); // Added by the script
        ATT_ASSERT(test_buffer, "Sequence codepoints: 2",
            "mjb_string_emoji_sequence test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Listed emoji sequence: %s",
            listed ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Listed emoji sequence: yes",
            "mjb_string_is_emoji_sequence test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "RGI emoji: %s",
            rgi ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "RGI emoji: yes",
            "mjb_string_is_rgi_emoji test failed") // Added by the script
    }

    {
        ...
        ATT_ASSERT(test_buffer, "Name: HANGUL SYLLABLE GAG",
            "mjb_hangul_syllable_name test failed") // Added by the script
    }

    {
        ...
        if(mjb_hangul_syllable_decomposition(0xAC01, decomposition) != MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Decomposition starts with: U+%04X",
            decomposition[0]); // Added by the script
        ATT_ASSERT(test_buffer, "Decomposition starts with: U+1100",
            "mjb_hangul_syllable_decomposition test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Composition: U+%04X",
            length == 1 ? characters[0].codepoint : 0); // Added by the script
        ATT_ASSERT(test_buffer, "Composition: U+AC01",
            "mjb_hangul_syllable_composition test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "U+754C is wide: %s",
            width == MJB_EAW_WIDE ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "U+754C is wide: yes",
            "mjb_codepoint_east_asian_width test failed") // Added by the script
    }

    {
        ...
        if(mjb_display_width(input, strlen(input), MJB_ENC_UTF_8, MJB_WIDTH_CONTEXT_WESTERN,
               &width) != MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Display columns: %zu",
            width); // Added by the script
        ATT_ASSERT(test_buffer, "Display columns: 3",
            "mjb_display_width test failed") // Added by the script
    }

    {
        ...
        if(mjb_locale_parse("sr-Latn-RS", 10, MJB_ENC_UTF_8, &locale, &error) != MJB_STATUS_OK) {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Locale: %s %s %s", locale.language,
            locale.script, locale.region); // Added by the script
        ATT_ASSERT(test_buffer, "Locale: sr Latn RS",
            "mjb_locale_parse test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer),
            "Turkish locale selected: yes"); // Added by the script
        ATT_ASSERT(test_buffer, "Turkish locale selected: yes",
            "mjb_locale_set test failed") // Added by the script
...

    {
        ...
        if(mjb_string_convert_encoding("A", 1, MJB_ENC_UTF_8, MJB_ENC_UTF_16LE, &result) !=
                MJB_STATUS_OK ||
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Result released: %s",
            result.output == NULL ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Result released: yes",
            "mjb_result_free test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Version is available: %s",
            version[0] != '\0' ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Version is available: yes",
            "mjb_version test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Version number is positive: %s",
            version > 0 ? "yes" : "no"); // Added by the script
        ATT_ASSERT(test_buffer, "Version number is positive: yes",
            "mjb_version_number test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer), "Unicode version: %s",
            version); // Added by the script
        ATT_ASSERT(test_buffer, "Unicode version: 17.0.0",
            "mjb_unicode_version test failed") // Added by the script
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer),
            "Standard allocator installed: yes"); // Added by the script
        ATT_ASSERT(test_buffer, "Standard allocator installed: yes",
            "mjb_set_memory_functions test failed") // Added by the script
        mjb_shutdown();
    }

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer),
            "Library state reset: yes"); // Added by the script
        ATT_ASSERT(test_buffer, "Library state reset: yes",
            "mjb_shutdown test failed") // Added by the script
    }
...

    {
        ...
        snprintf(test_buffer, sizeof(test_buffer),
            "Reallocation succeeded: yes"); // Added by the script
        ATT_ASSERT(test_buffer, "Reallocation succeeded: yes",
            "mjb_realloc test failed") // Added by the script
        mjb_free(larger);
    }
