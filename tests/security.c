...
int test_security(void *arg) {
    ...
    ATT_ASSERT(mjb_string_is_confusable("\xD0\x90", 2, "A", 1, MJB_ENC_UTF_8), true,
        "Cyrillic A confusable with Latin A")

    // Cyrillic "а" (U+0430) is confusable with Latin "a" (U+0061)
    // skeleton("а")="a", skeleton("a")="a"
    ATT_ASSERT(mjb_string_is_confusable("\xD0\xB0", 2, "a", 1, MJB_ENC_UTF_8), true,
        "Cyrillic a confusable with Latin a")

    // "A" (Latin capital) is NOT confusable with "a" (Latin lowercase)
    // skeleton("A")="A", skeleton("a")="a" → different
    ATT_ASSERT(mjb_string_is_confusable("A", 1, "a", 1, MJB_ENC_UTF_8), false,
        "A not confusable with a")

    // "a" is not confusable with "b"
    ATT_ASSERT(mjb_string_is_confusable("a", 1, "b", 1, MJB_ENC_UTF_8), false,
        "a not confusable with b")

    // A string is confusable with itself
    ATT_ASSERT(mjb_string_is_confusable("hello", 5, "hello", 5, MJB_ENC_UTF_8), true,
    ...
    ATT_ASSERT(mjb_string_is_confusable("hello", 5, "h\xD0\xB5llo", 6, MJB_ENC_UTF_8), true,
        "hello confusable with h(Cyrillic e)llo")

    // Empty strings: both have empty skeleton (output_size==0), returns false
    ATT_ASSERT(mjb_string_is_confusable("", 0, "", 0, MJB_ENC_UTF_8), false,
        "empty strings not confusable")

    // Different lengths with no possible match
    ATT_ASSERT(mjb_string_is_confusable("a", 1, "ab", 2, MJB_ENC_UTF_8), false,
        "a not confusable with ab")

    // Digit '1', capital 'I', and pipe '|' all map to skeleton 'l'
    ATT_ASSERT(mjb_string_is_confusable("1", 1, "l", 1, MJB_ENC_UTF_8), true,
        "1 confusable with l")
    ATT_ASSERT(mjb_string_is_confusable("I", 1, "l", 1, MJB_ENC_UTF_8), true,
        "I confusable with l")
    ATT_ASSERT(mjb_string_is_confusable("|", 1, "l", 1, MJB_ENC_UTF_8), true,
        "| confusable with l")
    ATT_ASSERT(mjb_string_is_confusable("1", 1, "I", 1, MJB_ENC_UTF_8), true,
        "1 confusable with I")

    // Digit '0' maps to skeleton 'O' (letter)
    ATT_ASSERT(mjb_string_is_confusable("0", 1, "O", 1, MJB_ENC_UTF_8), true,
        "0 confusable with O")
    ATT_ASSERT(mjb_string_is_confusable("0", 1, "o", 1, MJB_ENC_UTF_8), false,
        "0 not confusable with o (O != o)")

    // 'm' maps to a two-codepoint skeleton "rn": skeleton("m") == skeleton("rn")
    ATT_ASSERT(mjb_string_is_confusable("m", 1, "rn", 2, MJB_ENC_UTF_8), true,
        "m confusable with rn (multi-codepoint skeleton)")
    ATT_ASSERT(mjb_string_is_confusable("mm", 2, "rnrn", 4, MJB_ENC_UTF_8), true,
