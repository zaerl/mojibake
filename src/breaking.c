...
MJB_EXPORT char *mjb_line_break(const char *buffer, size_t length, mjb_encoding encoding) {
    ...
    char *breaks = malloc(real_length + 2);
    ...
    for(size_t i = 0; i < real_length; ++i) {
        ...
        if(!mjb_codepoint_line_breaking_class(buffer[i], (mjb_line_breaking_class*)&classes[i])) {
            continue;
        }
    ...
    for(size_t i = 0; i < real_length; ++i) {
        ...
        if(previous_c == MJB_LBC_CR && cc == MJB_LBC_LF) {
